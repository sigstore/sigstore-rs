...
impl FulcioClient {
    ...
    pub fn request_cert_v2(
        ...
        /*
        let sct = if sct_embedded {
            // PrecertificateSignedCertificateTimestamps isn't implemented yet in x509_cert
            // let 😭: ... = cert.tbs_certificate.get();
            todo!("tnytown: embedded SCT extraction");
        } else {
            if let SigningCertificate::SignedCertificateDetachedSct(cert) = response {
