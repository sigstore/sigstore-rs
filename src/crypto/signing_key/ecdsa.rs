...
mod tests {
    ...
    fn ecdsa_from_unencrypted_pem() {
        let content = fs::read("tests/data/keys/ecdsa_private.key").unwrap();
    ...
    fn ecdsa_from_encrypted_pem() {
        let content = fs::read("tests/data/keys/ecdsa_encrypted_private.key").unwrap();
    ...
    fn ecdsa_to_encrypted_pem() {
        let key = EcdsaKeys::<p256::NistP256>::new().unwrap();
    ...
    fn ecdsa_to_and_from_pem() {
        let key = EcdsaKeys::<p256::NistP256>::new().unwrap();
        let key = key.private_key_to_pem().unwrap();
    ...
    fn ecdsa_generate_public_key() {
        let key = EcdsaKeys::<p256::NistP256>::new();
        assert!(key.is_ok());
        let key = key.unwrap();
        ...
        assert!(CosignVerificationKey::from_der(&pubkey, &ECDSA_P256_SHA256_ASN1).is_ok());
    }
    ...
    fn ecdsa_sign_and_verify() {
        let key = EcdsaKeys::<p256::NistP256>::new().unwrap();
        let pubkey = key.public_key_to_pem().unwrap();
        let signer = EcdsaSigner::<_, sha2::Sha256>::from_ecdsa_keys(&key).unwrap();
