...
mod tests {
    ...
    fn ed25519_from_unencrypted_pem() {
        let content = fs::read("tests/data/keys/ed25519_private.key").unwrap();
    ...
    fn ed25519_from_encrypted_pem() {
        let content = fs::read("tests/data/keys/ed25519_encrypted_private.key").unwrap();
    ...
    fn ed25519_to_encrypted_pem() {
        let key = Ed25519Keys::new().unwrap();
    ...
    fn ed25519_generate_public_key() {
        ...
        assert!(CosignVerificationKey::from_der(&pubkey, &ED25519).is_ok());
    }
    ...
    fn ed25519_sign_and_verify() {
        let key = Ed25519Keys::new().unwrap();
        let pubkey = key.public_key_to_pem().unwrap();
        let signer = Ed25519Signer::from_ed25519_keys(&key).unwrap();
