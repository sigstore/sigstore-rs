        ...
        .and_then(|tvd| tvd.get("rfc3161Timestamps"))
    {
        if let Some(timestamps) = timestamp_data.as_array() {
            ...
            if let Some(first_ts) = timestamps.get(0) {
                if let Some(signed_ts_b64) = first_ts.get("signedTimestamp").and_then(|v| v.as_str()) {
                    println!("Found signedTimestamp field");

                    // Decode the timestamp
                    use base64::prelude::*;
                    let timestamp_der = BASE64_STANDARD.decode(signed_ts_b64)
                        .expect("Failed to decode timestamp");

                    println!("Timestamp DER length: {}", timestamp_der.len());
                    println!("First 32 bytes: {}", hex::encode(&timestamp_der[..32.min(timestamp_der.len())]));
                    ...
                    let signature_b64 = bundle.get("messageSignature")
                        .and_then(|ms| ms.get("signature"))
                        .and_then(|s| s.as_str())
                        .or_else(|| {
                            bundle.get("dsseEnvelope")
