...
fn test_rekor2_dsse_happy_path() {
    ...
    let bundle: Value = serde_json::from_str(&bundle_json)
        .expect("Failed to parse bundle JSON");
    ...
    if let Some(dsse) = bundle.get("dsseEnvelope") {
        ...
        if let Some(payload_b64) = dsse.get("payload").and_then(|v| v.as_str()) {
            ...
            println!("Payload: {}", serde_json::to_string_pretty(&payload_json).unwrap());

            // Check the subject digest
            if let Some(subject) = payload_json.get("subject").and_then(|s| s.as_array()).and_then(|a| a.get(0)) {
                if let Some(digest) = subject.get("digest").and_then(|d| d.get("sha256")).and_then(|v| v.as_str()) {
                    println!("Subject digest: {}", digest);

                    // The conformance test expects this digest
                    let expected_digest = "a0cfc71271d6e278e57cd332ff957c3f7043fdda354c4cbb190a30d56efa01bf";
    ...
    if let Some(timestamp_data) = bundle.get("verificationMaterial")
